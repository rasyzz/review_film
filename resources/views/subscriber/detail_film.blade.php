...
<body class="bg-gray-900 text-white">
    ...
    <div class="overflow-x-auto mt-35 md:mt-40">
        ...
        <div class="w-full h-auto bg-gray-900 p-5 flex flex-col space-y-6">
                    ...
                    <div class="testimonial-box bg-white text-black p-4 rounded-lg shadow-md border border-black mb-4">
                        ...
                        @if (Auth::check() && Auth::id() == $comment->user->id)
                            <div class="mt-2 flex space-x-2">
                                ...
                                <button type="button" onclick="showCommentDeleteModal(this)"
                                    class="text-black hover:text-gray-700 text-sm"
                                    data-id="{{ $comment->id_comments }}">
                                    <i class="fas fa-trash-alt"></i>
                                </button>

                                <!-- Form Delete (Tersembunyi) -->
                                <form id="commentDeleteForm" method="POST" class="hidden"
                                    data-action-base="{{ route('comments.destroy', '') }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
                    ...
                    <div class="p-6">
                        <div class="mb-5">
                            <div class="flex items-center mb-4">
                                ...
                                <p class="text-white">Anda yakin ingin menghapus komentar ini?</p>
                            </div>
                        </div>
                        <div class="flex justify-end gap-3">
                            <button onclick="closeCommentDeleteModal()"
                                class="px-4 py-2 border border-gray-300 rounded-lg text-white hover:bg-gray-50 hover:text-gray-700 transition-colors">
                                Tidak
                            </button>
                            <button onclick="confirmCommentDelete()"
            ...
            <!-- JavaScript for Comment Delete Modal and Toast -->
            <script>
                // Global variable to store the current comment ID
                let currentCommentId = null;

                function showCommentDeleteModal(button) {
                    // Get the ID from data attribute
                    currentCommentId = button.getAttribute('data-id');
                ...
                function confirmCommentDelete() {
                    if (!currentCommentId) {
                        alert("Terjadi kesalahan: ID komentar tidak ditemukan.");
                        return;
                    }

                    // Ambil elemen form dan atur action URL
                    const deleteForm = document.getElementById('commentDeleteForm');
                    const baseUrl = deleteForm.getAttribute('data-action-base'); // Ambil URL dasar dari Blade
                    deleteForm.action = baseUrl + '/' + currentCommentId;
